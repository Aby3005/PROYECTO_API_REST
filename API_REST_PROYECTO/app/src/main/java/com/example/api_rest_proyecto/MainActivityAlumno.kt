...
class MainActivityAlumno : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val retrofit = Retrofit.Builder()
            .baseUrl("http://192.168.0.3/api/alumno.php/")
                    ...
                    if (alumnos != null) {
                        ...
                        adapter.setOnItemClickListener(object : AlumnoAdapter.OnItemClickListener {
                            override fun onItemClick(alumno: Alumno) {
                                ...
                                AlertDialog.Builder(this@MainActivityAlumno)
    ...
    private fun eliminarAlumno(alumno: Alumno, api: AlumnoApi) {
        ...
        llamada.enqueue(object : Callback<Void> {
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                if (response.isSuccessful) {
                    Toast.makeText(this@MainActivityAlumno, "Alumno eliminado", Toast.LENGTH_SHORT).show()
                    cargarDatos(api)
                } else {
                    val error = response.errorBody()?.string()
                    Log.e("API", "Error al eliminar alumno : $error")
                    Toast.makeText(this@MainActivityAlumno, "Error al eliminar alumno 1", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onFailure(call: Call<Void>, t: Throwable) {
                Log.e("API", "Error al eliminar alumno : $t")
                Toast.makeText(this@MainActivityAlumno, "Error al eliminar alumno 2", Toast.LENGTH_SHORT).show()
            }
        })
    }
}